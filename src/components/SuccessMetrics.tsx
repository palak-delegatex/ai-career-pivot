...
const metrics = [
  { value: 500, suffix: "+" },
  { value: 92, suffix: "%" },
  { value: 3.2, suffix: "x", decimals: 1 },
  { value: 15, prefix: "$", suffix: "K" },
];
...
export default function SuccessMetrics() {
  const t = useTranslations("successMetrics");
  const ref = useRef(null);
  const inView = useInView(ref, { once: true, margin: "-80px" });

  const metricLabels = t.raw("metricLabels") as string[];
        ...
        >
          {metrics.map((m, i) => (
            ...
            >
              ...
              <p className="text-slate-400 text-sm">{metricLabels[i]}</p>
