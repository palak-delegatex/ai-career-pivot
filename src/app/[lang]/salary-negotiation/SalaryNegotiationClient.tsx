...
type Tab = "overview" | "research" | "market" | "roleplay" | "talking-points" | "counter-offer" | "readiness";
...
interface MarketData {
  ...
  growthLabel: string;
  source: string;
  locationAdjustedMedian?: number;
  locationMultiplier?: number;
  locationName?: string;
  industryPremium?: number;
  industryName?: string;
  experienceAdjustedMedian?: number;
...
export default function SalaryNegotiationClient() {
  ...
  async function generateResearch() {
    ...
    try {
      const res = await fetch("/api/salary-negotiation/research", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          ...
          targetSalary: offer.base ? parseFloat(offer.base) : undefined,
        }),
      });
      ...
      while (true) {
        ...
        setResearchReport(content);
      }
      setResearchReport(content);
    } catch {
      setResearchReport("Sorry, I had trouble generating the research report. Please try again.");
    } finally {
  ...
  const TABS: { id: Tab; label: string; icon: React.ReactNode }[] = [
    ...
    { id: "research", label: "Salary Research", icon: <Search className="w-4 h-4" /> },
    ...
    return (
      <main className="max-w-2xl mx-auto px-6 py-16">
        ...
        <div className="space-y-6">
          ...
          {/* Skills, unique value, and industry */}
          <div className="space-y-3">
            <div>
            ...
            <div>
              <label className="text-xs text-slate-500 mb-1 block">Industry / Company Type</label>
              <input type="text" value={industry} onChange={(e) => setIndustry(e.target.value)} placeholder="Big Tech, Finance, Startup, Healthcare, etc." className="w-full px-3 py-2.5 rounded-lg bg-slate-800 border border-slate-700 text-white placeholder-slate-600 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500" />
            </div>
          </div>

          {/* Action buttons — Research first */}
          <button
            onClick={generateResearch}
            disabled={!effectiveRole}
            className="w-full flex items-center justify-center gap-2 px-4 py-4 rounded-xl bg-gradient-to-r from-blue-600 to-emerald-600 hover:from-blue-500 hover:to-emerald-500 font-bold text-base transition-colors disabled:opacity-50 disabled:cursor-not-allowed shadow-lg shadow-blue-900/30"
          >
            <Search className="w-5 h-5" /> Start Salary Research
          </button>
        ...
        <div className="flex-1 overflow-y-auto">
          <div className="max-w-3xl mx-auto px-6 py-8">
            <div className="flex items-center gap-3 mb-2">
              ...
              <h2 className="text-2xl font-bold">Salary Research Report</h2>
            </div>
            <p className="text-slate-400 text-sm mb-6">
              Comprehensive market analysis for <span className="text-white font-medium">{effectiveRole}</span>
              {location && <span> in <MapPin className="w-3 h-3 inline" /> {location}</span>}
              {industry && <span> · <Building2 className="w-3 h-3 inline" /> {industry}</span>}
              {yearsExp && <span> · <Briefcase className="w-3 h-3 inline" /> {yearsExp}yr exp</span>}
            </p>
            {researchLoading && !researchReport ? (
              <div className="flex items-center gap-3 text-slate-400 py-12 justify-center">
                <Loader2 className="w-5 h-5 animate-spin" /> Generating your salary research report…
              </div>
            ) : researchReport ? (
              <div className="space-y-6">
                <div className="bg-slate-800/40 rounded-2xl border border-slate-700 p-6 text-sm text-slate-200">
                  <div className="space-y-0.5">{renderMarkdown(researchReport)}</div>
                </div>
                <div className="rounded-xl bg-slate-800/40 border border-slate-700 p-5 space-y-3">
                  <h3 className="font-semibold text-sm">Continue Your Preparation</h3>
                  <div className="grid grid-cols-2 gap-3">
                    <button
                      onClick={() => { fetchMarket(); setTab("market"); }}
                      className="flex items-center gap-2 px-4 py-3 rounded-xl bg-blue-600/20 border border-blue-500/30 text-blue-400 hover:bg-blue-600/30 text-sm font-medium transition-colors"
                    >
                      <TrendingUp className="w-4 h-4" /> View Market Data
                    </button>
                    <button
                      onClick={generateTalkingPoints}
                      className="flex items-center gap-2 px-4 py-3 rounded-xl bg-teal-600/20 border border-teal-500/30 text-teal-400 hover:bg-teal-600/30 text-sm font-medium transition-colors"
                    >
                      <FileText className="w-4 h-4" /> Get Talking Points
                    </button>
                    <button
                      onClick={startRoleplay}
                      className="flex items-center gap-2 px-4 py-3 rounded-xl bg-purple-600/20 border border-purple-500/30 text-purple-400 hover:bg-purple-600/30 text-sm font-medium transition-colors"
                    >
                      <MessageSquare className="w-4 h-4" /> Practice Role-Play
                    </button>
                    <button
                      onClick={() => { if (!marketData) fetchMarket(); setTab("counter-offer"); }}
                      disabled={!offer.base}
                      className="flex items-center gap-2 px-4 py-3 rounded-xl bg-amber-600/20 border border-amber-500/30 text-amber-400 hover:bg-amber-600/30 text-sm font-medium transition-colors disabled:opacity-40"
                    >
                      <Calculator className="w-4 h-4" /> Counter-Offer
                    </button>
                  </div>
              ...
              <div className="text-center py-12">
                <p className="text-slate-500 mb-4">Fill in your role details and generate a comprehensive salary research report.</p>
                <button
          ...
          <div className="max-w-3xl mx-auto px-6 py-8">
            ...
            {marketLoading ? (
              ...
              <div className="space-y-8">
                ...
                {/* Location, Industry, Experience adjustments */}
                {(marketData.locationAdjustedMedian || marketData.industryName || marketData.experienceAdjustedMedian) && (
                  <div className="space-y-3">
                    <h3 className="font-semibold text-sm text-slate-300">Adjusted Salary Estimates</h3>
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                      {marketData.locationAdjustedMedian && (
                        <div className="bg-slate-800/60 rounded-xl p-4 border border-blue-500/20">
                          <div className="flex items-center gap-1.5 mb-1">
                            <MapPin className="w-3.5 h-3.5 text-blue-400" />
                            <span className="text-xs text-blue-400">{marketData.locationName}</span>
                          </div>
                          <div className="text-lg font-bold text-blue-300">{fmt(marketData.locationAdjustedMedian)}</div>
                          <div className="text-xs text-slate-500">
                            {marketData.locationMultiplier != null && marketData.locationMultiplier >= 1
                              ? `+${Math.round((marketData.locationMultiplier - 1) * 100)}%`
                              : `${Math.round(((marketData.locationMultiplier ?? 1) - 1) * 100)}%`} vs national
                          </div>
                        </div>
                      )}
                      {marketData.industryName && (
                        <div className="bg-slate-800/60 rounded-xl p-4 border border-violet-500/20">
                          <div className="flex items-center gap-1.5 mb-1">
                            <Building2 className="w-3.5 h-3.5 text-violet-400" />
                            <span className="text-xs text-violet-400">{marketData.industryName}</span>
                          </div>
                          <div className="text-lg font-bold text-violet-300">
                            {marketData.industryPremium != null ? `${marketData.industryPremium >= 0 ? "+" : ""}${Math.round(marketData.industryPremium * 100)}%` : "—"}
                          </div>
                          <div className="text-xs text-slate-500">industry premium</div>
                        </div>
                      )}
                      {marketData.experienceAdjustedMedian && (
                        <div className="bg-slate-800/60 rounded-xl p-4 border border-amber-500/20">
                          <div className="flex items-center gap-1.5 mb-1">
                            <Briefcase className="w-3.5 h-3.5 text-amber-400" />
                            <span className="text-xs text-amber-400">{marketData.experienceLevel}</span>
                          </div>
                          <div className="text-lg font-bold text-amber-300">{fmt(marketData.experienceAdjustedMedian)}</div>
                          <div className="text-xs text-slate-500">adjusted target</div>
                        </div>
                      )}
                    </div>
                  </div>
                ...
                {offer.base && (
                  <div className="rounded-xl bg-slate-800/40 border border-slate-700 p-5">
                    ...
                    {(() => {
                      const base = parseFloat(offer.base);
                      const compareMedian = marketData.experienceAdjustedMedian ?? marketData.locationAdjustedMedian ?? marketData.salaryMedian;
                      const compareLabel = marketData.experienceAdjustedMedian ? "adjusted target" : marketData.locationAdjustedMedian ? "location-adjusted median" : "median";
                      const pctOfMedian = Math.round((base / compareMedian) * 100);
                      const diff = base - compareMedian;
                      return (
                        <p className="text-sm text-slate-300">
                          ...
                          <span className={diff >= 0 ? "text-emerald-400 font-semibold" : "text-amber-400 font-semibold"}>
                            {pctOfMedian}% of {compareLabel}
                          </span>
                          {diff < 0 ? ` (${fmt(Math.abs(diff))} below — strong case for a counter)` : ` (${fmt(diff)} above — focus on equity/perks)`}.
                        </p>
                      );
                    })()}
                  </div>
